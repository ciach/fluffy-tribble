...
from orchestrator.generation.models import (
    SelectorViolation, AuditResult, SelectorType, ViolationSeverity
)
...
"""
    ...
    def test_init_with_default_policy(self, auditor):
        ...
        assert auditor.policy_rules["preferred_selectors"] == ["getByRole", "getByLabel", "getByTestId"]
...
"""
        ...
        css_violations = [v for v in result.violations if v.selector_type == SelectorType.CSS]
        xpath_violations = [v for v in result.violations if v.selector_type == SelectorType.XPATH]
...
"""
        ...
        assert result.is_compliant is True  # Justified violations don't block compliance
...
"""
        ...
        brittle_selectors = [v.selector for v in css_violations 
                           if auditor._is_brittle_css_selector(v.selector)]
        assert len(brittle_selectors) >= 2  # Should detect nth-child and long class names
    
    def test_has_justification_comment(self, auditor):
        """Test justification comment detection."""
        assert auditor._has_justification_comment("// This is justified for legacy support")
        assert auditor._has_justification_comment("/* Necessary for third-party integration */")
        assert auditor._has_justification_comment("await page.locator('.btn'); // Required by design")
        assert not auditor._has_justification_comment("await page.locator('.btn'); // Regular comment")
    ...
    def test_is_brittle_css_selector(self, auditor):
        ...
        assert auditor._is_brittle_css_selector(".very-long-generated-class-name-12345678901234567890")
        assert auditor._is_brittle_css_selector("#very-long-generated-id-12345678901234567890")
    ...
    def test_selector_patterns_compilation(self, auditor):
        ...
        role_match = patterns["getByRole"].search("getByRole('button', { name: 'Submit' })")
