...
from orchestrator.models.parsers import (
    ResponseParser,
    ParsedResponse,
    ParsedResponseType,
)
...
class TestModelInteractionManager:
    ...
    @patch("orchestrator.models.utilities.ModelRateLimitManager")
    @pytest.mark.asyncio
    async def test_execute_task_context_too_large(
        self,
        mock_rate_manager,
        mock_context,
        mock_parser,
    ...
    async def test_validate_setup(
        ...
        # Setup mocks
        mock_router_instance = Mock()
        mock_router_instance.get_available_providers.return_value = [
