...
class TestCodePatcher:
    ...
    async def test_apply_patch_success(
        ...
        with patch.object(code_patcher, '_validate_patch_preconditions', new_callable=AsyncMock), \
             patch.object(code_patcher, '_create_backup', new_callable=AsyncMock, return_value="/backup/path"), \
             patch.object(code_patcher, '_validate_patched_file', new_callable=AsyncMock) as mock_validate:
    ...
    async def test_apply_patch_validation_failure(
        ...
        with patch.object(code_patcher, '_validate_patch_preconditions', new_callable=AsyncMock), \
             patch.object(code_patcher, '_create_backup', new_callable=AsyncMock, return_value="/backup/path"), \
             patch.object(code_patcher, '_validate_patched_file', new_callable=AsyncMock) as mock_validate, \
             patch.object(code_patcher, '_rollback_patch', new_callable=AsyncMock) as mock_rollback:
    ...
    @pytest.mark.asyncio
    async def test_detect_flaky_test(
        self, 
        code_patcher,
        mock_filesystem_client
    ):
