...
def mock_selector_auditor():
    ...
    auditor.audit_test_code.return_value = AuditResult(
        is_compliant=True, violations=[], total_selectors=3, compliant_selectors=3
    )
...
def sample_test_plan():
    ...
    test_step = TestStep(
        action="click", target="login button", description="Click the login button"
    )
...
class TestTestGenerator:
    ...
    def test_init_with_default_auditor(self, mock_model_router):
        ...
        with patch(
            "orchestrator.generation.generator.SelectorAuditor"
    ...
    def test_generate_test_success(
        self,
        mock_model_router,
        mock_selector_auditor,
        sample_test_plan,
        sample_generated_test_content,
    ):
    ...
    def test_generate_test_audit_failure(
        self,
        mock_model_router,
        mock_selector_auditor,
        sample_test_plan,
...
"""
    ...
    def test_scaffold_page_object(
        self, mock_model_router, mock_selector_auditor, sample_test_plan
    ):
...
"""
    ...
    def test_model_router_error_handling(
        self, mock_model_router, mock_selector_auditor, sample_test_plan
    ):
