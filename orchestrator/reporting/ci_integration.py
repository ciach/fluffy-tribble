...
class ApprovalManager:
    ...
    async def _wait_for_approval(self, request_id: str, timeout: int) -> bool:
        ...
        if os.getenv("CI"):
            self.logger.warning(
                "Running in CI without auto-approval, denying operation"
            )
            return False
        ...
        self.logger.info(
            "Simulating user approval (would be interactive in real implementation)"
        )
...
class CIIntegrator:
    ...
    async def send_notification(
        self, payload: NotificationPayload, webhook_url: Optional[str] = None
    ) -> bool:
        """
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        self.logger.warning(
                            f"Notification failed with status {response.status}"
                        )
                        return False
...
"""
        ...
        failed_tests = [
            test for test in report.test_cases if test.status.value == "failed"
        ]
        if failed_tests:
            summary += "### ❌ Failed Tests\n\n"
            for test in failed_tests[:5]:  # Limit to first 5
                summary += (
                    f"- `{test.name}` - {test.error_message or 'No error message'}\n"
                )
    ...
    def _generate_artifacts_section(self, report: TestRunReport) -> str:
        ...
        for artifact_type, artifacts in artifacts_by_type.items():
            ...
            if len(artifacts) > 3:
                section += (
                    f"- ... and {len(artifacts) - 3} more {artifact_type} files\n"
                )
...
class GitOperationApprovalWorkflow:
    ...
    async def commit_with_approval(
        self, files: List[str], message: str, auto_approve: bool = False
    ) -> bool:
        """
