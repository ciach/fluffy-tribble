...
class ArtifactInfo(BaseModel):
    ...
    def validate_type(cls, v):
        valid_types = [
            "screenshot", "trace", "video", "log", "report", 
            "coverage", "performance", "network", "console"
        ]
...
class TestRunReport(BaseModel):
    ...
    artifacts: List[ArtifactInfo] = Field(default_factory=list, description="Test artifacts")
    ...
    ci_info: Optional[Dict[str, Any]] = Field(None, description="CI environment information")
...
class CIIntegrationConfig(BaseModel):
    ...
    retention_days: int = Field(30, ge=1, le=365, description="Artifact retention in days")
...
class ApprovalRequest(BaseModel):
    ...
    files_affected: List[str] = Field(default_factory=list, description="Files to be modified")
    ...
    rejection_reason: Optional[str] = Field(None, description="Rejection reason if denied")
