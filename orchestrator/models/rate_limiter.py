...
class RateLimiter:
    ...
    async def _wait_for_rate_limit(self, estimated_tokens: int) -> None:
        ...
        while time.time() - start_time < max_wait_time:
            ...
            can_proceed = (
                requests_last_minute < self.config.requests_per_minute
                and requests_last_hour < self.config.requests_per_hour
                and tokens_last_minute + estimated_tokens
                <= self.config.tokens_per_minute
                and tokens_last_hour + estimated_tokens <= self.config.tokens_per_hour
            )
    ...
    def _count_requests_in_window(self, window_seconds: int) -> int:
        ...
        return sum(
            1 for record in self.request_history if record.timestamp >= cutoff_time
        )
    ...
    def _calculate_wait_time(
        ...
        if tokens_minute + estimated_tokens > self.config.tokens_per_minute:
            ...
            for record in sorted_records:
                ...
                if (
                    tokens_minute - cumulative_tokens + estimated_tokens
                    <= self.config.tokens_per_minute
                ):
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                if not self._should_retry(error_type, attempt):
                    logger.error(
                        f"Not retrying error type '{error_type}' on attempt {attempt + 1}"
                    )
                    raise e
...
class ModelRateLimitManager:
    ...
    def _setup_default_configs(self) -> None:
        ...
        openai_retry = RetryConfig(
            max_retries=3, base_delay=1.0, strategy=RetryStrategy.EXPONENTIAL_BACKOFF
        )

        ollama_retry = RetryConfig(
            max_retries=2, base_delay=0.5, strategy=RetryStrategy.LINEAR_BACKOFF
        )
