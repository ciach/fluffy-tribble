...
class ContextManager:
    ...
    def _get_encoding(self, model_name: str) -> tiktoken.Encoding:
        ...
        try:
            ...
            logger.warning(
                f"Failed to get encoding for {model_name}, using default: {e}"
            )
            return tiktoken.get_encoding("cl100k_base")
    ...
    def count_tokens(self, text: str) -> int:
        ...
        try:
            ...
            logger.warning(
                f"Failed to count tokens, using character approximation: {e}"
            )
    ...
    def prepare_context(
        ...
        logger.info(
            f"Context too large ({total_tokens} tokens), applying {strategy.value} strategy"
        )
    ...
    def _create_context_chunks(
        self, context: Dict[str, Any], task_type: TaskType
    ) -> List[ContextChunk]:
    ...
    def _truncate_context(
        self, chunks: List[ContextChunk], window: ContextWindow
    ) -> Dict[str, Any]:
    ...
    def _prioritize_context(
        self, chunks: List[ContextChunk], window: ContextWindow, task_type: TaskType
    ) -> Dict[str, Any]:
            ...
            if chunk.token_count <= remaining_tokens:
                ...
                if chunk.priority >= 7:  # Only summarize important content
                    ...
                    if summary_tokens <= remaining_tokens:
                        result[chunk.metadata["original_key"]] = (
        ...
        if content_type == "test_code":
            ...
            for line in lines[:20]:  # First 20 lines
                if any(
                    ...
                    for keyword in ["test(", "describe(", "expect(", "await"]
                ):
