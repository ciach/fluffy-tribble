...
from .types import TaskType, ModelProvider, ModelResponse, ModelConfig, ModelRoutingRule
...
class ModelRouter:
    ...
    def _setup_clients(self) -> None:
        ...
        try:
            ...
            if self.config.model_provider in ["ollama", "mixed"]:
                ...
                logger.info(
                    f"LiteLLM configured for Ollama at {self.config.ollama_base_url}"
                )
    ...
    def _setup_routing_rules(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Model routing configured with {len(self.routing_rules)} rules"
            )
    ...
    async def _call_openai_model(
        self, config: ModelConfig, messages: List[Dict[str, str]]
    ) -> Dict[str, Any]:
    ...
    async def route_task(
        ...
        logger.info(
            f"Routing {task_type.value} task to {rule.primary_config.provider.value}"
        )
        ...
        try:
            ...
            logger.info(
                f"Successfully completed {task_type.value} task with {rule.primary_config.provider.value}"
            )
            return response
            ...
            if rule.fallback_config:
                logger.info(
                    f"Attempting fallback to {rule.fallback_config.provider.value}"
                )

                try:
                    result = await self._call_model_with_retry(
                        rule.fallback_config, messages
                    )
                    ...
                    logger.info(
                        f"Successfully completed {task_type.value} task with fallback {rule.fallback_config.provider.value}"
                    )
                    return response
                    ...
                    raise ModelError(
                        f"Both primary and fallback models failed. Primary: {e}, Fallback: {fallback_error}"
                    )
            else:
                raise e
            ...
            "routing_rules": [
                {
                    ...
                    "fallback_provider": (
                        rule.fallback_config.provider.value
                        if rule.fallback_config
                        else None
                    ),
                }
