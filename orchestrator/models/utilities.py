...
class ModelInteractionManager:
    ...
    async def execute_task(
        ...
        try:
            ...
            if not fits:
                logger.warning(f"Context size ({total_tokens}) exceeds limit ({max_tokens})")
            ...
            execution_time = (datetime.now() - start_time).total_seconds()
            logger.error(f"Task {task_type.value} failed after {execution_time:.2f}s: {e}")
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "templates": {
                "available_templates": [t.value for t in self.template_manager.list_available_templates()]
            },
