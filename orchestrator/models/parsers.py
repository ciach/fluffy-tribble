...
class ResponseParser:
    ...
    def parse_response(self, model_response: ModelResponse) -> ParsedResponse:
        """
        ...
        if not parser:
            logger.warning(
                f"No parser available for task type: {model_response.task_type}"
            )
    ...
    def _parse_planning_response(self, content: str) -> ParsedResponse:
        ...
        objectives = self._extract_list_section(
            content, r"(?:Test Objectives|Objectives):(.*?)(?=\n\d+\.|$)", "objectives"
        )
        test_cases = self._extract_test_cases(content)
        data_requirements = self._extract_list_section(
            content,
            r"(?:Test Data Requirements|Data Requirements):(.*?)(?=\n\d+\.|$)",
            "data requirements",
        )
        coverage_analysis = self._extract_text_section(
            content, r"(?:Coverage Analysis):(.*?)(?=\n\d+\.|$)"
        )
        risk_assessment = self._extract_text_section(
    ...
    def _parse_debugging_response(self, content: str) -> ParsedResponse:
        ...
        root_cause = self._extract_text_section(
            content, r"(?:Root [Cc]ause|Root Cause Analysis):(.*?)(?=\n\d+\.|$)"
        )
        code_changes = self._extract_code_changes(content)
        explanation = self._extract_text_section(
            content, r"(?:Explanation):(.*?)(?=\n\d+\.|$)"
        )
        prevention_suggestions = self._extract_list_section(
            content,
    ...
    def _parse_drafting_response(self, content: str) -> ParsedResponse:
        ...
        test_file_content = self._extract_code_block(
            content, "typescript"
        ) or self._extract_code_block(content, "ts")
    ...
    def _parse_analysis_response(self, content: str) -> ParsedResponse:
        ...
        health_assessment = self._extract_text_section(
            content, r"(?:Health Assessment|Overall.*Assessment):(.*?)(?=\n\d+\.|$)"
        )
        patterns = self._extract_list_section(
            content, r"(?:Patterns|Anti-patterns):(.*?)(?=\n\d+\.|$)", "patterns"
        )
        recommendations = self._extract_list_section(
            content, r"(?:Recommendations):(.*?)(?=\n\d+\.|$)", "recommendations"
        )
        priorities = self._extract_list_section(
            content, r"(?:Priority|Priorities):(.*?)(?=\n\d+\.|$)", "priorities"
        )
    ...
    def _parse_generation_response(self, content: str) -> ParsedResponse:
        ...
        code_content = self._extract_code_block(
            content, "typescript"
        ) or self._extract_first_code_block(content)
    ...
    def _extract_test_cases(self, content: str) -> List[Dict[str, Any]]:
        ...
        case_pattern = (
            r"(?:Test Case|Case)\s*\d*:?\s*(.+?)(?=(?:Test Case|Case)\s*\d*:|$)"
        )
        ...
        for match in matches:
            ...
            if case_text:
                ...
                steps = self._extract_list_section(
                    case_text, r"(?:Steps|Test Steps):(.*?)(?=Expected|$)", "steps"
                )
                expected = self._extract_text_section(
                    case_text, r"(?:Expected|Expected Results?):(.*?)$"
                )
