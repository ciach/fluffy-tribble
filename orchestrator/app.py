...
def main() -> int:
    ...
    try:
        ...
        logger.info("QA Operator starting up", extra={
            "metadata": {
        ...
        if workflow_manager:
            logger = get_logger("qa_operator.main")
            logger.error(f"Configuration validation failed: {e.message}", extra={
                "metadata": e.to_dict()
            })
        ...
        if workflow_manager:
            logger = get_logger("qa_operator.main")
            logger.error(f"QA Operator error: {e.message}", extra={
                "metadata": e.to_dict()
            })
            workflow_manager.end_workflow(success=False, error=e)
        else:
