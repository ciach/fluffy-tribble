...
class MCPToolResponse(BaseModel):
    ...
    duration: Optional[float] = Field(
        None, ge=0, description="Call duration in seconds"
    )
...
class MCPServerInfo(BaseModel):
    ...
    status: ServerStatus = Field(
        ServerStatus.UNKNOWN, description="Current server status"
    )
...
class FileOperationRequest(BaseModel):
    ...
    create_backup: bool = Field(
        True, description="Whether to create backup before modification"
    )
...
class FileOperationResult(BaseModel):
    ...
    bytes_affected: Optional[int] = Field(
        None, ge=0, description="Number of bytes read/written"
    )
    ...
    error: Optional[str] = Field(None, description="Error message if failed")
    duration: Optional[float] = Field(
        None, ge=0, description="Operation duration in seconds"
    )
...
class BrowserAction(BaseModel):
    ...
    options: Dict[str, Any] = Field(
        default_factory=dict, description="Additional action options"
    )
...
class BrowserActionResult(BaseModel):
    ...
    error: Optional[str] = Field(None, description="Error message if failed")
    duration: Optional[float] = Field(
        None, ge=0, description="Action duration in seconds"
    )
