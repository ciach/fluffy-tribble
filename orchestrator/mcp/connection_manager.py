...
class MCPConnectionManager:
    ...
    async def connect_server(self, server_name: str) -> bool:
        ...
        for attempt in range(server_config.max_retries + 1):
            try:
                ...
                if attempt > 0:
                    delay = server_config.retry_delay * (server_config.retry_backoff ** (attempt - 1))
                    self.logger.debug(f"Retrying connection to {server_name} in {delay:.1f}s (attempt {attempt + 1})")
                ...
                error_msg = f"Connection attempt {attempt + 1} failed for {server_name}: {e}"
                ...
                if attempt == server_config.max_retries:
                    ...
                    self.logger.error(f"Failed to connect to MCP server {server_name} after {server_config.max_retries + 1} attempts")
                    raise MCPConnectionError(
    ...
    async def _create_mcp_connection(self, server_config: MCPServerConfig) -> Any:
        ...
        self.logger.debug(f"Creating MCP connection: {server_config.command} {' '.join(server_config.args)}")
    ...
    async def connect_all_servers(self) -> Dict[str, bool]:
        ...
        for server_name in self._servers:
            task = asyncio.create_task(
                self._safe_connect_server(server_name),
        ...
        self.logger.info(f"Connected to {successful_connections}/{len(results)} MCP servers")
    ...
    async def disconnect_all_servers(self) -> None:
        ...
        for server_name in list(self._connections.keys()):
            task = asyncio.create_task(
                ...
                name=f"disconnect_{server_name}"
            )
    ...
    def get_health_status(self, server_name: Optional[str] = None) -> Dict[str, ConnectionHealth]:
        """
        ...
        """
        if server_name:
            return {server_name: self._health.get(server_name)} if server_name in self._health else {}
        ...
        for server_name, health in self._health.items():
            if health.status == ConnectionStatus.FAILED:
                ...
                if health.next_retry is None or current_time >= health.next_retry:
                    ...
                    if server_name not in self._reconnect_tasks or self._reconnect_tasks[server_name].done():
    ...
    async def _reconnect_server(self, server_name: str) -> None:
        ...
        try:
            ...
            if success:
                self.logger.info(f"Successfully reconnected to MCP server: {server_name}")
            else:
                # Schedule next retry
                server_config = self._servers[server_name]
                delay = server_config.retry_delay * (server_config.retry_backoff ** health.retry_count)
    ...
    def get_connected_servers(self) -> List[str]:
        ...
        return [name for name, health in self._health.items() 
