...
class GitStatus(BaseModel):
    ...
    ahead_behind: Optional[Dict[str, int]] = Field(
        None, description="Commits ahead/behind remote"
    )
...
class GitOperationResult(BaseModel):
    ...
    files_affected: List[str] = Field(
        default_factory=list, description="Files affected by operation"
    )
...
class GitMCPClient:
    ...
    async def _ensure_connection(self) -> Any:
        ...
        if not self.is_available():
            self.logger.warning(
                "Git MCP server not connected, attempting to connect..."
            )
            try:
    ...
    async def _call_git_tool(
        self, operation: GitOperationType, arguments: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.logger.debug(
                f"Calling Git MCP tool: {operation.value} with args: {arguments}"
            )
    ...
    async def _mock_git_response(
        ...
        if operation == GitOperationType.STATUS:
            ...
            return {"success": True, "files_added": arguments.get("files", [])}
    ...
    async def stage_files(self, files: List[str]) -> GitOperationResult:
        ...
        response = await self._call_git_tool(GitOperationType.ADD, {"files": files})
    ...
    async def create_pull_request(
        self, pr_info: PullRequestInfo, include_workflow_context: bool = True
    ) -> GitOperationResult:
        """
    ...
    async def validate_repository(self) -> ValidationResult:
        ...
        if not self.is_available():
            result.add_warning(
                "Git MCP server not available - operations will use fallback"
            )
