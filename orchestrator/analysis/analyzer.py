...
class FailureAnalyzer:
    ...
    async def analyze_failure(
        ...
        if execution_result.status == TestStatus.PASSED:
            raise ValidationError(
                "Cannot analyze successful test execution", validation_type="input"
            )
        ...
        try:
            ...
            analysis_result = await self._perform_comprehensive_analysis(
                execution_result, stack_trace_analysis, artifact_analyses, context or {}
            )
    ...
    async def _analyze_stack_trace(
        self, execution_result: ExecutionResult, analysis_id: str
    ...
    async def _analyze_single_artifact(
        self, artifact_metadata, analysis_id: str
        ...
        try:
            ...
            if artifact_metadata.artifact_type == ArtifactType.CONSOLE_LOG:
                findings, error_indicators = await self._analyze_console_logs(
                    artifact_path
                )
            elif artifact_metadata.artifact_type == ArtifactType.NETWORK_LOG:
                findings, error_indicators = await self._analyze_network_logs(
                    artifact_path
                )
            elif artifact_metadata.artifact_type == ArtifactType.TRACE:
                findings, error_indicators = await self._analyze_trace_file(
                    artifact_path
                )
    ...
    async def _analyze_console_logs(
        self, log_path: Path
    ...
    async def _analyze_network_logs(
        self, log_path: Path
    ) -> Tuple[List[str], List[str]]:
    ...
    async def _perform_comprehensive_analysis(
        ...
        analysis_prompt = self._build_analysis_prompt(
            execution_result, stack_trace_analysis, artifact_analyses, context
        )
        ...
        try:
            ...
            result = {
                "primary_error_category": ErrorCategory(
                    analysis_data["primary_error_category"]
                ),
                "secondary_categories": [
                    ErrorCategory(cat)
                    for cat in analysis_data.get("secondary_categories", [])
                ],
            ...
            for fix_data in analysis_data.get("fix_suggestions", []):
                fix_suggestion = FixSuggestion(
                    ...
                    requires_manual_review=fix_data.get(
                        "requires_manual_review", False
                    ),
                )
