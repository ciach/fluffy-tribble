...
class ArtifactAnalysis(BaseModel):
    ...
    artifact_type: str = Field(..., description="Type of artifact (trace, screenshot, etc.)")
    findings: List[str] = Field(default_factory=list, description="Key findings from this artifact")
    ...
    relevant_sections: List[Dict[str, Any]] = Field(
        ...
        description="Relevant sections or timestamps"
    )
...
class StackTraceAnalysis(BaseModel):
    ...
    failing_line: Optional[int] = Field(None, description="Line number where error occurred")
    failing_code: Optional[str] = Field(None, description="Code that caused the failure")
    playwright_action: Optional[str] = Field(None, description="Playwright action that failed")
...
class FixSuggestion(BaseModel):
    ...
    # Code changes
    original_code: Optional[str] = Field(None, description="Original code to be replaced")
    ...
    potential_side_effects: List[str] = Field(
        default_factory=list, 
        description="Potential negative impacts of this fix"
    )
    test_impact: str = Field(
        default="minimal", 
        description="Expected impact on test behavior"
    )
...
class FailureAnalysis(BaseModel):
    ...
    analysis_duration: float = Field(..., ge=0, description="Time taken for analysis in seconds")
    
    # Error categorization
    primary_error_category: ErrorCategory = Field(..., description="Primary error category")
    secondary_categories: List[ErrorCategory] = Field(
        default_factory=list, 
        description="Additional error categories"
    )
    ...
    artifact_analyses: List[ArtifactAnalysis] = Field(
        default_factory=list, 
        description="Analysis of test artifacts"
    )
    ...
    fix_suggestions: List[FixSuggestion] = Field(
        default_factory=list, 
        description="Suggested fixes ordered by confidence"
    )
    ...
    similar_failures: List[str] = Field(
        default_factory=list, 
        description="Similar failures seen in the past"
    )
    ...
    def has_high_confidence_fixes(self) -> bool:
        ...
        return any(fix.confidence == ConfidenceLevel.HIGH for fix in self.fix_suggestions)
...
class PatchValidationResult(BaseModel):
    ...
    semantic_changes: List[str] = Field(
        default_factory=list, 
        description="Semantic changes detected"
    )
...
class CodePatch(BaseModel):
    ...
    end_line: Optional[int] = Field(None, ge=1, description="Ending line number for block operations")
    ...
    requires_backup: bool = Field(default=True, description="Whether to create backup before applying")
    requires_validation: bool = Field(default=True, description="Whether to validate after applying")
    dry_run_tested: bool = Field(default=False, description="Whether patch was tested in dry run")
    ...
    def validate_line_range(cls, v, values):
        ...
        if operation in [PatchOperation.REPLACE_BLOCK, PatchOperation.DELETE_BLOCK]:
            if v is None:
                raise ValueError(f"end_line is required for {operation.value} operations")
    ...
    def affects_multiple_lines(self) -> bool:
        ...
        return self.operation in [
            PatchOperation.REPLACE_BLOCK, 
            PatchOperation.DELETE_BLOCK
...
class PatchResult(BaseModel):
    ...
    applied_at: datetime = Field(
        default_factory=datetime.utcnow, 
        description="When patch was applied"
    )
    ...
    # Error information
    error_message: Optional[str] = Field(None, description="Error message if patch failed")
...
class FlakyTestDetection(BaseModel):
    ...
    flakiness_score: float = Field(..., ge=0.0, le=1.0, description="Flakiness score (0-1)")
    confidence: ConfidenceLevel = Field(..., description="Confidence in flaky detection")
    
    # Flakiness patterns
    detected_patterns: List[str] = Field(
        default_factory=list, 
        description="Detected flaky patterns"
    )
    failure_frequency: Optional[float] = Field(
        None, ge=0.0, le=1.0, 
        description="Historical failure frequency"
    )
    
    # Stability improvements
    stability_suggestions: List[FixSuggestion] = Field(
        default_factory=list, 
        description="Suggestions to improve test stability"
    )
