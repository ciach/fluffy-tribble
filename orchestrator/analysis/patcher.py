...
class CodePatcher:
    ...
    async def apply_patch(
        ...
        try:
            ...
            lines_changed = self._calculate_lines_changed(original_content, patched_content)
            ...
            if result.needs_rollback and not dry_run:
                logger.warning(f"Patch {patch.patch_id} needs rollback due to validation failures")
    ...
    async def create_patch_from_suggestion(
        ...
        try:
            ...
            logger.info(f"Created patch {patch.patch_id} for {fix_suggestion.fix_type.value}")
            return patch
            
        except Exception as e:
            logger.error(f"Failed to create patch from suggestion: {e}")
            raise ValidationError(
                f"Patch creation failed: {e}",
                validation_type="patch_creation"
            )
    ...
    async def detect_flaky_test(
        ...
        try:
            ...
            if failure_history:
                ...
                failures = sum(1 for run in failure_history if not run.get('success', True))
            ...
            raise CodePatcherError(
                f"Flaky test detection failed: {e}",
    ...
    def _check_playwright_best_practices(self, content: str) -> List[str]:
        ...
        if re.search(r'page\.waitForTimeout\(\d+\)', content):
            warnings.append("Hard-coded timeout found - consider using element-based waits")
        ...
        for selector in css_selectors:
            if selector.startswith('.') or selector.startswith('#') or ' ' in selector:
                warnings.append(f"CSS selector found: {selector} - consider semantic selectors")
    ...
    async def _generate_patched_code(
        ...
        if fix_suggestion.fix_type == FixType.WAIT_CONDITION:
            ...
            return f"{original_code}  // TODO: Apply {fix_suggestion.fix_type.value} fix"
    ...
    def _generate_selector_replacement_fix(self, original_code: str) -> str:
        ...
        if 'locator(' in original_code and ('#' in original_code or '.' in original_code):
            # Suggest getByTestId instead
            return original_code.replace('locator(', 'getByTestId(').replace('#', '').replace('.', '')
    ...
    async def _analyze_flaky_patterns(
        self, 
        content: str, 
        test_name: str
    ) -> List[str]:
    ...
    def _calculate_flakiness_score(
        ...
        if failure_history:
            failure_rate = sum(1 for run in failure_history if not run.get('success', True))
