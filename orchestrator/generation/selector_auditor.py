...
class SelectorAuditor:
    ...
    def _compile_selector_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "getByPlaceholder": re.compile(r"getByPlaceholder\s*\(\s*['\"]([^'\"]+)['\"]"),
            "getByText": re.compile(r"getByText\s*\(\s*['\"]([^'\"]+)['\"]"),
            "css": re.compile(r"locator\s*\(\s*['\"]([^'\"//]+)['\"]"),  # Exclude xpath patterns
            "xpath": re.compile(r"locator\s*\(\s*['\"]//([^'\"]*)['\"]"),
            "querySelector": re.compile(r"querySelector\s*\(\s*['\"]([^'\"]+)['\"]"),
            "querySelectorAll": re.compile(r"querySelectorAll\s*\(\s*['\"]([^'\"]+)['\"]")
        }
    ...
    def audit_test_code(self, test_code: str, file_path: str = "") -> AuditResult:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            has_justification = (self._has_justification_comment(line) or 
                               self._has_justification_comment(prev_line))
            
            line_violations, line_selectors, line_compliant, line_justified = self._audit_line(
                line, line_num, has_justification
            )
        ...
        is_compliant = len([v for v in violations if v.severity == ViolationSeverity.ERROR]) == 0
        ...
        logger.info(f"Audit completed: {result.compliance_rate:.1f}% compliant, "
                   f"{len(violations)} violations")
        
        return result
    
    def _audit_line(self, line: str, line_num: int, has_justification: bool = False) -> tuple[List[SelectorViolation], int, int, int]:
    ...
    def _check_selector_compliance(self, selector_name: str, selector_value: str, 
                                 selector_type: SelectorType, line_num: int,
                                 has_justification: bool) -> Optional[SelectorViolation]:
        ...
        if selector_name in self.policy_rules["discouraged_selectors"]:
            ...
            if has_justification and self.policy_rules.get("allow_with_justification", False):
        ...
        if selector_type == SelectorType.CSS and self._is_brittle_css_selector(selector_value):
            return SelectorViolation(
                line_number=line_num,
                selector=selector_value,
                selector_type=selector_type,
    ...
    def validate_compliance(self, audit_result: AuditResult) -> bool:
        ...
        """
        error_violations = [v for v in audit_result.violations 
