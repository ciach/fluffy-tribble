...
class Config:
    ...
    headless_mode: Optional[bool] = field(
        default_factory=lambda: (
            True if os.getenv("QA_OPERATOR_HEADLESS", "").lower() == "true"
            else None  # Will be determined by CI mode if not explicitly set
        )
    )
    
    # Logging configuration
    log_level: str = field(default_factory=lambda: os.getenv("QA_OPERATOR_LOG_LEVEL", "INFO"))
    log_format: str = field(default_factory=lambda: "json" if os.getenv("CI", "").lower() == "true" else "text")
    
    # Model configuration
    model_provider: str = field(default_factory=lambda: os.getenv("QA_OPERATOR_MODEL_PROVIDER", "mixed"))
    openai_api_key: Optional[str] = field(default_factory=lambda: os.getenv("OPENAI_API_KEY"))
    ollama_base_url: str = field(default_factory=lambda: os.getenv("OLLAMA_BASE_URL", "http://localhost:11434"))
    
    # Artifact management
    artifact_retention_days: int = field(
        default_factory=lambda: int(os.getenv("QA_OPERATOR_ARTIFACT_RETENTION_DAYS", "30" if os.getenv("CI", "").lower() == "true" else "7"))
    )
    ...
    # MCP configuration
    mcp_config_path: Path = field(default_factory=lambda: Path.cwd() / "orchestrator" / "mcp.config.json")
    ...
    def validate(self) -> None:
        ...
        if self.model_provider in ["openai", "mixed"] and not self.openai_api_key:
            errors.append("OPENAI_API_KEY environment variable is required for OpenAI model usage")
