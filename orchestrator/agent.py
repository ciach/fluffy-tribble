...
class WorkflowStateManager:
    ...
    def transition_to(
        self, new_state: WorkflowState, metadata: Optional[Dict[str, Any]] = None
    ) -> None:
...
class QAOperatorAgent:
    ...
    async def _initialize_mcp_connections(self) -> None:
        ...
        self.playwright_client = PlaywrightMCPClient(
            self.mcp_manager, logger=self.logger
        )

        self.filesystem_client = FilesystemMCPClient(
            self.mcp_manager, e2e_dir=Path("e2e"), logger=self.logger
        )
    ...
    def _initialize_model_router(self) -> None:
        ...
        self.model_router = ModelRouter(config=self.config, logger=self.logger)
    ...
    def _initialize_components(self) -> None:
        ...
        self.test_generator = TestGenerator(
            self.model_router, selector_auditor=selector_auditor
        )

        # Initialize artifact manager
        self.artifact_manager = ArtifactManager(
            config=self.config, workflow_id=None  # Will be set when workflow starts
        )
        ...
        self.failure_analyzer = FailureAnalyzer(
            config=self.config, model_router=self.model_router
        )

        # Initialize code patcher
        self.code_patcher = CodePatcher(
            config=self.config, filesystem_client=self.filesystem_client
        )
            ...
            self.logger.error(
                ...
                extra={
                    "metadata": {
                        ...
                        "error_type": e.__class__.__name__,
                        "duration": (
                            self._current_workflow.duration
    ...
    async def _execute_workflow_phases_with_recovery(
        self, specification: Union[str, Dict[str, Any], TestSpecification]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            execution_results = await self._execute_execution_phase_with_recovery(
                generated_tests
            )
            ...
            results["phases"]["execution"] = {
                ...
                "passed": len(
                    [r for r in execution_results if r.status == TestStatus.PASSED]
                ),
                "failed": len(
                    [r for r in execution_results if r.status == TestStatus.FAILED]
                ),
                "duration": time.time() - phase_start,
                "status": "success",
            }
            ...
            self.logger.warning(
                f"Execution phase failed, continuing with available results: {str(e)}"
            )
            execution_results = []
        ...
        failed_tests = [
            ...
            if hasattr(r, "status") and r.status == TestStatus.FAILED
        ]
        if failed_tests:
            try:
                ...
                patch_results = (
                    await self._execute_analysis_and_patching_phase_with_recovery(
                        failed_tests
                    )
                )
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    ...
                    self.logger.error(
                        f"Planning failed after {max_retries + 1} attempts"
                    )

        raise last_error

    async def _execute_generation_phase_with_retry(
        self, test_plan: Any, max_retries: int = 2
    ) -> List[Any]:
        ...
        for test in generated_tests:
            try:
                ...
                failed_result = ExecutionResult(
                    test_name=str(test),
                    test_file=(
                        test.file_path if hasattr(test, "file_path") else str(test)
                    ),
    ...
    async def _execute_analysis_and_patching_phase_with_recovery(
        self, failed_tests: List[ExecutionResult]
    ) -> List[Any]:
                ...
                if hasattr(analysis, "suggested_fixes") and analysis.suggested_fixes:
                    for fix in analysis.suggested_fixes:
                        try:
                            ...
                            patch = (
                                await self.code_patcher.create_patch_from_suggestion(
                                    fix, failed_test.test_file
                                )
                            )
                            ...
                            if patch_result.success:
                                try:
                                    rerun_result = (
                                        await self.test_executor.execute_test(
    ...
    async def _attempt_workflow_recovery(self, error: Exception) -> None:
        ...
        self.logger.info(
            f"Attempting workflow recovery (attempt {self._state_manager.recovery_attempts})"
        )
    ...
    async def _execute_workflow_phases(
        ...
        results["phases"]["planning"] = {
            "test_plan": (
                test_plan.to_dict() if hasattr(test_plan, "to_dict") else str(test_plan)
            ),
            "duration": time.time() - phase_start,
        }
        ...
        if failed_tests:
            ...
            patch_results = await self._execute_analysis_and_patching_phase(
                failed_tests
            )
            results["phases"]["analysis_patching"] = {
                "failures_analyzed": len(failed_tests),
                ...
                extra={
                    "metadata": {
                        "test_cases": (
                            ...
                            if hasattr(test_plan, "test_cases")
                            else 0
                        ),
        ...
        try:
            ...
            if hasattr(test_plan, "test_cases") and test_plan.test_cases:
                for i, test_case in enumerate(test_plan.test_cases):
                    file_name = (
                        f"test_{test_case.name.lower().replace(' ', '_')}.spec.ts"
                    )
                    generated_test = self.test_generator.generate_test(
                        test_plan, file_name=file_name, output_dir="e2e"
                    )
                    generated_tests.append(generated_test)
            else:
                # Fallback: generate single test from plan
                file_name = (
                    f"test_{test_plan.specification_id.replace('-', '_')}.spec.ts"
                )
                generated_test = self.test_generator.generate_test(
                    test_plan, file_name=file_name, output_dir="e2e"
                )
    ...
    def _log_performance_metrics(self) -> None:
        ...
        model_metrics = (
            self.model_router.get_usage_metrics() if self.model_router else {}
        )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        status = {
            "ready": self.is_ready,
            "current_workflow": (
                self._current_workflow.workflow_id if self._current_workflow else None
            ),
            "components": {
